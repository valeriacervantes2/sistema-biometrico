...
class AccountView(ctk.CTkFrame):
    ...
    def cargar_datos_desde_bd(self):
        """Consume los datos desde el servicio de configuración local"""
        try:
            fila = obtener_perfil()
            if fila:
                self.datos["nombre"] = fila[0] if fila[0] else ""
                self.datos["a_paterno"] = fila[1] if fila[1] else ""
                self.datos["a_materno"] = fila[2] if fila[2] else ""
                self.datos["correo"] = fila[3] if fila[3] else ""
                self.datos["telefono"] = fila[4] if fila[4] else ""
                self.datos["facultad"] = fila[5] if fila[5] else ""
    ...
    def guardar_datos_perfil(self):
        # 1. Guardar la Foto en la carpeta local si fue actualizada
        ...
        # 2. Guardar textos en la base de datos local
        ...
        # Refrescar la pantalla de vuelta al modo lectura
        self.crear_vista_lectura()