...
class DashboardView(ctk.CTkFrame):
    ...
    def create_sidebar(self):
        ...
        self.sidebar_frame = ctk.CTkFrame(self, width=sidebar_width, corner_radius=0, fg_color=COLORS["sidebar"], border_width=1, border_color=COLORS["border"])
    ...
    # SOLO compacta si es sidebar real, no overlay
        es_overlay = hasattr(self, "overlay_sidebar") and master == self.overlay_sidebar

        if self.is_compact and not es_overlay:
            texto_final = texto.split(" ")[0]
        else:
            texto_final = texto
